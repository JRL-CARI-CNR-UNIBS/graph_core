...
double trj_execution_thread_frequency;
double replanning_thread_frequency;
...
double collision_checker_thread_frequency;
double k_freq;
std::string test;
...
void replanning_fcn()
{
  replanning_thread_frequency = k_freq*replanning_thread_frequency;
  ros::Rate lp(replanning_thread_frequency);
  ...
  while (!stop)
  {
    ...
    if(replan)
    {
      ...
      if(path_obstructed)
      {
        replan_offset=(dt_replan_restricted-dt)*1.2;
        time_informedOnlineRepl = 0.90*dt_replan_restricted;
        string_dt = " dt ridotto";
        computing_avoiding_path = true;
      }
      else
      {
        replan_offset=(dt_replan_relaxed-dt)*1.2;
        time_informedOnlineRepl = 0.90*dt_replan_relaxed;
...
void collision_check_fcn()
{
  ...
  collision_checker_thread_frequency = k_freq*collision_checker_thread_frequency;

  ros::Rate lp(collision_checker_thread_frequency);
...
int main(int argc, char **argv)
{
  ...
  for(int n_iter = init_test; n_iter<end_test; n_iter++)
  {
    ...
    t=0.0;
    dt=0.01;
    trj_execution_thread_frequency = 1/dt;
    replanning_thread_frequency = 1/dt_replan_restricted;
    replan_offset=(dt_replan_restricted-dt)*2;
    t_replan=t+replan_offset;
    n_conn = 0;
    n_conn_replan = 0;
    first_replan = true;
    ...
    trj_execution_thread_frequency = k_freq*trj_execution_thread_frequency;
    ros::Rate lp(trj_execution_thread_frequency);
