...
int main(int argc, char **argv) {
  ...
  cnr_logger::TraceLoggerPtr logger = std::make_shared<cnr_logger::TraceLogger>(
      "node_connection_test", file_path);
  ...
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::GREEN() << "q1 " << q1.transpose());
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::GREEN() << "q2 " << q2.transpose());

  CNR_INFO(logger, cnr_logger::RESET() << cnr_logger::BOLDGREEN() << "Done!");

  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::WHITE()
                       << "--- Create nodes on these configurations ---");
  ...
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::WHITE() << "--- Display the nodes ---");
  ...
  CNR_INFO(logger,
           cnr_logger::RESET()
               << cnr_logger::WHITE()
               << "--- Create and add a connection between the nodes ---");
  graph::core::ConnectionPtr connection =
      std::make_shared<graph::core::Connection>(parent, child, logger);
  ...
  bool valid = ((parent->getParentConnectionsSize() == 0) &&
                (parent->getChildConnectionsSize() == 1) &&
                (parent->getNetParentConnectionsSize() == 0) &&
                (parent->getNetChildConnectionsSize() == 0) &&
                (child->getParentConnectionsSize() == 1) &&
                (child->getChildConnectionsSize() == 0) &&
                (child->getNetParentConnectionsSize() == 0) &&
                (child->getNetChildConnectionsSize() == 0));
  if (not valid) {
  ...
  q3.setRandom();
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::GREEN() << "q3 " << q3.transpose());

  graph::core::NodePtr net_parent =
      std::make_shared<graph::core::Node>(q3, logger);
  graph::core::ConnectionPtr net_connection =
      std::make_shared<graph::core::Connection>(net_parent, child, logger,
                                                true);
  ...
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::YELLOW() << "net connection: \n"
                       << *net_connection);
  ...
  valid = ((parent->getParentConnectionsSize() == 0) &&
           (parent->getChildConnectionsSize() == 1) &&
           (parent->getNetParentConnectionsSize() == 0) &&
           (parent->getNetChildConnectionsSize() == 0) &&
           (net_parent->getParentConnectionsSize() == 0) &&
           (net_parent->getChildConnectionsSize() == 0) &&
           (net_parent->getNetParentConnectionsSize() == 0) &&
           (net_parent->getNetChildConnectionsSize() == 1) &&
           (child->getParentConnectionsSize() == 1) &&
           (child->getChildConnectionsSize() == 0) &&
           (child->getNetParentConnectionsSize() == 1) &&
           (child->getNetChildConnectionsSize() == 0));
  ...
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::WHITE()
                       << "--- Switch net and standard connections ---");
  child->switchParentConnection(net_connection);

  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::YELLOW() << "connection (now net): \n"
                       << *connection);
  CNR_INFO(logger, cnr_logger::RESET() << cnr_logger::YELLOW()
                                       << "net connection (now not net): \n"
                                       << *net_connection);

  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::BLUE() << "parent (now net): \n"
                       << *parent);
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::BLUE() << "net parent (now not net): \n"
                       << *net_parent);
  CNR_INFO(logger, cnr_logger::RESET() << cnr_logger::CYAN() << "child:  \n"
                                       << *child);

  valid = ((parent->getParentConnectionsSize() == 0) &&
           (parent->getChildConnectionsSize() == 0) &&
           (parent->getNetParentConnectionsSize() == 0) &&
           (parent->getNetChildConnectionsSize() == 1) &&
           (net_parent->getParentConnectionsSize() == 0) &&
           (net_parent->getChildConnectionsSize() == 1) &&
           (net_parent->getNetParentConnectionsSize() == 0) &&
           (net_parent->getNetChildConnectionsSize() == 0) &&
           (child->getParentConnectionsSize() == 1) &&
           (child->getChildConnectionsSize() == 0) &&
           (child->getNetParentConnectionsSize() == 1) &&
           (child->getNetChildConnectionsSize() == 0));
  ...
  CNR_INFO(logger,
           cnr_logger::RESET()
               << cnr_logger::WHITE()
               << "--- Disconnect child to remove all the connections ---");
  child->disconnect();
  ...
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::WHITE() << "--- Remove connection ---");
  connection->remove();
  ...
  CNR_INFO(logger, cnr_logger::RESET()
                       << cnr_logger::WHITE()
                       << "--- Check parallelism of connections ---");
  if (connection->isParallel(net_connection)) {
    CNR_FATAL(logger, "something went wrong with connections parallelism");
    throw std::runtime_error(
        "something went wrong with connections parallelism");
  }

  Eigen::VectorXd q_mid = (q1 - q2) / 2;
  graph::core::NodePtr node_mid =
      std::make_shared<graph::core::Node>(q_mid, logger);
  graph::core::ConnectionPtr connection_parallel =
      std::make_shared<graph::core::Connection>(parent, node_mid, logger);
