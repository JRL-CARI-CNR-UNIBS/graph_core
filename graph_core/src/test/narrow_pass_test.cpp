...
int main(int argc, char **argv)
{
  ...
  if (!nh.getParam("reward",rewards))
  {
    ROS_ERROR("reward is not set");
    return 0;
  }
  ...
  for (const double& reward: rewards)
  {
    for (const double& tube_radius: tube_radii)
    {
      for (const double& forgetting_factor: forgetting_factors)
      {
        ...
        if (tube_sampler)
          ROS_INFO("TUBE: reward=%f, forgetting_factor%f,tube radius=%f",reward,forgetting_factor,tube_radius);
        else
          ROS_INFO("ELLIPSE");
        ...
        for (const int& dof: dofs)
        {
          ...
          if (tube_sampler)
          {
            ...
            description_file << "reward: " << reward<<std::endl;
          }
          else
          {
            ...
            for (;iter<maximum_iter;iter++)
            {
              ...
              if (solver->update(solution))
              {
                ...
                if (tube_sampler)
                {
                  ...
                  tube_sampler->setRadius(tube_radius*solution->cost());
                  if (used_tube_sampler)
                    local_probability=std::min(1.,forgetting_factor*local_probability+reward*(delta_cost));
                }
              }
              else
              {
  ...
  ros::WallTime t0=ros::WallTime::now();
  while ((ros::WallTime::now()-t0)<ros::WallDuration(30))
