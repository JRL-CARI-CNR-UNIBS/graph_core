...
{
  assert(q.size()==factor.size());
  std::vector<double> result(q.size());
  for (unsigned int idx=0;idx<q.size();idx++)
    result.at(idx)=q.at(idx)*factor.at(idx);
  return result;
}
...
{
  return std::sqrt(squareDistance(q1,q2));
}
...
std::vector<std::vector<double>> interpolate(const std::vector<double> &q1, const std::vector<double> &q2, const double& distance_step)
{
  ...
  for (unsigned int ipnt=1;ipnt<npnt;ipnt++)
  {
    ...
    for (size_t i_dof=0;i_dof<q1.size();i_dof++)
      q.at(i_dof)=q1.at(i_dof)+(q2.at(i_dof)-q1.at(i_dof))*((double)ipnt)/((double)npnt);
