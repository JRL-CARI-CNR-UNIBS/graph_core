...
namespace pathplan
{
                               ...
                               int &connected2path_number)
    {
        ...
        for(unsigned int j = 0; j< admissible_other_paths_.size(); j++)
        {
            ...
            for(const NodePtr& path2_node : path2_node_vector)
            {
                ...
                if (distance_path_node < diff_subpath_cost && diff_subpath_cost >= 0) //if the Euclidean distance between the two nodes is bigger than the maximum cost for the connecting_path to be convenient, it isuseless to calculate a connecting_path because it surely will not be convenient
                {
                    ...
                    if (!solver_->addStart(path1_node_fake))
                    {
                      ROS_ERROR("unable to add start");
                      throw std::invalid_argument("unable to add start");
                    }

                    if (!solver_->addGoal(path2_node_fake))
                    {
                      ROS_ERROR("unable to add goal");
                      throw std::invalid_argument("unable to add goal");
                    }
    ...
    //Per il test
    bool Replanner::pathSwitchTest(const PathPtr &current_path,
                               const NodePtr &node,
                               const bool &succ_node,
                               PathPtr &new_path,
                               PathPtr &subpath_from_path2,
                               int &connected2path_number, pathplan::TestUtil& ut)
    {
        ...
        for(unsigned int j = 0; j< admissible_other_paths_.size(); j++)
        {
            ...
            if((path2_node_vector.back()->getConfiguration() - path2->getConnections().back()->getChild()->getConfiguration()).norm()<1e-06) //if the cloest node is the GOAL, the second closest node is considered
            {
                PathPtr path_support = path2->getSubpathToNode(path2->getConnections().back()->getParent());
                path2_node_vector.clear();
                path2_node_vector.push_back(path_support->findCloserNode(path1_node->getConfiguration()));
                ...
                if (diff_subpath_cost > 0 && distance_path_node < diff_subpath_cost) //if the Euclidean distance between the two nodes is bigger than the maximum cost for the connecting_path to be convenient, it isuseless to calculate a connecting_path because it surely will not be convenient
                {
                    ...
                    if (solver_->solve(connecting_path, 1000))
                    {
                        ...
                        if(conn_cost<path_cost && conn_cost<subpath1_cost && !subpath2.empty()) //maybe "if(conn_cost<path_cost)" is enough (the GOAL is always excluded and at the beginning subpath1_cost is path_cost)
                        {
                            ...
                            std::vector<double> marker_scale(3,0.02);
                            ...
                            wp_state_vector = ut.displayTrajectoryOnMoveitRviz(new_path, t_vector, rviz_visual_tools::YELLOW, 1);
