...
double Connection::getWeigthedLength()
{
  return m_length*(1+m_params.weigth*(m_child->getCost()+m_parent->getCost()));
}
...
void Connection::computeLength()
{
  ...
  m_length=std::sqrt(m_square_length);
}
