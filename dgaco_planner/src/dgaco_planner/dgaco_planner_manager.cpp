...
bool DgacoPlannerManager::initialize(const moveit::core::RobotModelConstPtr& model, const std::string& ns)
{
  ...
  {
    ROS_DEBUG("Group name = %s, planner name: %s",p.first.c_str(),p.second.c_str());
    std::shared_ptr<DgacoPlanner> ptr(new DgacoPlanner(ns+"/"+p.second,p.first,model));
    m_planners.insert(std::pair<std::string,std::shared_ptr<ha_planner::DgacoPlanner>>(p.first,ptr));
